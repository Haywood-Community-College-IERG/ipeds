...
# Get all the credential seekers. We will use this to determine which students are in HS.
student_credential_seekers_all <- credential_seekers( ipeds_report_year, exclude_hs = FALSE ) %>%
    rename( Campus_ID = ID )

student_credential_seekers_nohs_all <- credential_seekers( ipeds_report_year, exclude_hs = TRUE ) %>%
    rename( Campus_ID = ID )

# These are the students who were credential seekers in at least 1 term the past year
student_credential_seekers <- student_credential_seekers_nohs_all %>%
    summarise( Credential_Seeker = max(Credential_Seeker), 
               .by = c( Campus_ID ) ) %>%
    mutate( type = "College" )

# These are all the students who were dual enrolled the entire past year
student_high_school <- setdiff( student_credential_seekers_all, student_credential_seekers_nohs_all ) %>%
    summarise( Credential_Seeker = max(Credential_Seeker), 
               .by = c( Campus_ID ) ) %>%
    select( -Credential_Seeker ) %>%
    anti_join( student_credential_seekers, by = join_by(Campus_ID) )  %>%
    mutate( Credential_Seeker = 2,
            type = "HS" )

students <- bind_rows( student_credential_seekers, student_high_school )
...
student_enrollment_all <- student_term_enrollment %>%
    ...
    left_join( students, by="Campus_ID" ) %>%
    ...
    mutate( type = case_when(
                !is.null(type) & !is.na(type) ~ type,
                substr(Term_Cohort_Abbrev,1,1) == 'H' ~ "HS",
                .default = "College"
            ),
            de_level = case_when(
                ...
                TRUE ~ 1) ) %>%
    mutate( Term_Cohort_Abbrev = case_when(
                .default = Term_Cohort_Abbrev,
                type == "HS" ~ str_c('H',substr(Term_Cohort_Abbrev,2,2))
            ) ) %>%
...
se_cohorts <- student_enrollment %>%
    select(Campus_ID,Term_ID,Term_Cohort_Abbrev,Credential_Seeker,DE,IPEDS_Race_Gender_Code,LINE_e = type) %>%
...
race_gender_cols <- tibble( FYRACE01=NA_character_, FYRACE02=NA_character_,
...
ipeds_e12_a <- se_cohorts %>%
    ...
    summarise( Students = sprintf("%06d", sum(student) ), 
               .by = c( LINE, IPEDS_Race_Gender_Code ) ) %>%
...
ipeds_e12_c <- se_cohorts %>%
    ...
    bind_rows( tibble(ENROLL_EXCLUSIVE=character(), ENROLL_SOME=character()) ) %>%
...
ipeds_e12_e <- se_cohorts %>%
    filter( LINE_e == "HS" ) %>%
    select( Campus_ID, IPEDS_Race_Gender_Code ) %>%
    distinct() %>%
    mutate( student = 1 ) %>%
    ### <b>
    #group_by( LINE, IPEDS_Race_Gender_Code ) %>%
    ...
    mutate( UNITID = cfg$school$ipeds,
            ...
            PART = 'E',
            Filler_1 = strrep(' ',9) ) %>%
    select( all_of(names(e12_e_cols)) )
...
if (WRITE_OUTPUT) {
    ...
    write.table( data.frame(ipeds_e12_d), 
                 ...
                 quote=FALSE, 
                 append=TRUE  )
    write.table( data.frame(ipeds_e12_e), 
                 file.path(output_path, fn_E12), 
                 sep="", 
                 col.names = FALSE, 
                 row.names = FALSE, 

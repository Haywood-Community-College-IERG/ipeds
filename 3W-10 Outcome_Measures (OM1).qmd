...
credential_terms <- terms %>%
    filter( Term_Reporting_Year >= report_data_year,
            Term_Reporting_Year <= report_data_year + 7 )
...
acad_credentials <- getColleagueData( "ACAD_CREDENTIALS" ) %>%
    filter( ACAD.INSTITUTIONS.ID == local(instid),
            coalesce(ACAD.TERM,'')!='',
    ...
    inner_join( terms %>% filter( Term_Reporting_Year >= report_data_year) %>% select(Term_ID, Term_End_Date ) ) %>%
...
not_graduated <- sac_cohort %>%
    filter( Graduation_Date > as.Date(str_c(report_data_year+8,'-08-31')) ) %>%
...
still_enrolled <- sac_load_by_term %>%
    filter( Term_Reporting_Year == report_data_year + 8,
            Semester == "FA" ) %>%
    select( ID ) %>%
    distinct() %>%
...
sac_last_enrollment <- not_graduated %>%
    ...
    left_join( sac_load_by_term %>% 
                   ...
                   left_join( terms %>% select(Term_ID, Term_Start_Date) ) %>%
                   filter( Term_Start_Date <= as.Date(str_c(report_data_year+8,'-08-31')) ) ) %>%
...
# This is needed for Part D
#
nsc_detail_pat_se <- str_glue(".*_DETLRPT_SE_.*_{fn_report_code}_{report_year}_se\\.csv")
transferred <- list.files( path = local_nsc_path,
                           pattern = nsc_detail_pat_se, 
                           full.names = TRUE, 
                           ignore.case = TRUE ) %>% 
    lapply(function(x) readr::read_csv(x, col_types = cols(.default = col_character()))) %>% 
    bind_rows %>%
    separate( col=`Requester Return Field`, into=c("ID","Cohort_NSC"), sep=";" ) %>%
    ...
    inner_join( ipeds_cohorts_ids, by = "ID" ) %>%
    ...
    inner_join( sac_last_enrollment, by = "ID" ) %>%
    ...
    anti_join( sac_load_by_term %>%
                   ...
                   select( ID ) %>%
                   distinct() %>%
                   inner_join( not_graduated, by = "ID" ), 
               by = "ID" ) %>%
...
transferred_summary <- transferred %>%
    group_by( LINE, RECIPIENT_TYPE ) %>%
    summarise( ENROLLED_ANOTHER = sprintf("%06d", n()), .groups = "keep" )
...
ipeds_om1_b <- sac_cohort %>%
    filter( Graduation_Date <= as.Date(str_c(report_data_year+4,'-08-31')) ) %>%
...
ipeds_om1_c <- sac_cohort %>%
    filter( Graduation_Date <= as.Date(str_c(report_data_year+6,'-08-31')) ) %>%
    group_by( ID, LINE, RECIPIENT_TYPE ) %>%
    summarise( Credential_Level = max(Credential_Level) ) %>%
    mutate( Associate = if_else(Credential_Level==10000,1,0),
            Certificate = if_else(Credential_Level==1,1,0) ) %>%
...
ipeds_om1_a %>% 
    ungroup() %>%
    select( LINE, REC_TYPE=RECIPIENT_TYPE, COHORT, EXCLUSION ) %>%
    mutate_at(c('LINE','REC_TYPE'),as.integer) %>%
    mutate_at(c('COHORT','EXCLUSION'), as.integer) %>%
    gt()
...
ipeds_om1_b %>% 
    ungroup() %>%
    select( LINE, REC_TYPE=RECIPIENT_TYPE, CERT=AWARD_CERTIFICATE, ASSOC=AWARD_ASSOCIATES, BACH=AWARD_BACHELORS ) %>%
    mutate_at(c('LINE','REC_TYPE'),as.integer) %>%
    mutate_at(c('CERT','ASSOC','BACH'), as.integer) %>%
    gt()
...
ipeds_om1_d %>%
    ungroup() %>%
    select( LINE, REC_TYPE=RECIPIENT_TYPE, ST_ENRL=STILL_ENROLLED, AN_ENRL=ENROLLED_ANOTHER) %>%
    mutate_at(c('LINE','REC_TYPE'),as.integer) %>%
    mutate_at(c('ST_ENRL','AN_ENRL'), as.integer) %>%
    gt()
```
...
if (WRITE_OUTPUT) {
    write.table( data.frame(ipeds_om1_a), file.path(output_path, fn), sep="", col.names = FALSE, row.names = FALSE, quote=FALSE  )
    write.table( data.frame(ipeds_om1_b), file.path(output_path, fn), sep="", col.names = FALSE, row.names = FALSE, quote=FALSE, append=TRUE  )
    write.table( data.frame(ipeds_om1_c), file.path(output_path, fn), sep="", col.names = FALSE, row.names = FALSE, quote=FALSE, append=TRUE  )
    write.table( data.frame(ipeds_om1_d), file.path(output_path, fn), sep="", col.names = FALSE, row.names = FALSE, quote=FALSE, append=TRUE  )
}
```
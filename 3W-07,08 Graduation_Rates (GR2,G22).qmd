---
title: "IPEDS Graduation Rates and Graduation Rates 200"
#format: pdf
format: html
#  docx:
...
nsc_detail_pat <- str_glue(".*_DETLRPT_SE_.*_hcc_ipeds_{fn_report_code}_{report_year}_se.*csv")
...
ipeds_cohort <- getColleagueData( "ipeds_cohorts", schema="local", version="latest" ) %>%
    ...
    inner_join( terms %>% select(Term_ID,Cohort_Start_Date=Term_Start_Date),
                by = "Term_ID") %>%
    mutate(Term = substr(Term_ID, 5, 6)) %>%  # Extract the term part (SU or FA) from Term_ID
    group_by(ID) %>%
    filter(!(any(Term == "FA") & Term == "SU")) %>%
    ungroup() %>%
    select(-Term)  # Remove the temporary Term column
...
person <- getColleagueData( "PERSON" ) %>%
    ...
    select( ID, 
            ...
            VISA.TYPE ) %>%
    collect() %>%
    inner_join( ipeds_cohort %>% select(ID), by = join_by(ID) ) %>%
...
student_programs <- getColleagueData( "STUDENT_PROGRAMS" ) %>%
    ...
    collect() %>%
    inner_join( student_programs__dates, by = join_by(ID, Program, EffectiveDatetime) ) %>%
    select( -EffectiveDatetime ) %>%
    inner_join( ipeds_cohort %>% select(ID, Cohort_Start_Date, IPEDS_Report), by = join_by(ID) ) %>%
    ...
    left_join( acad_programs, by = join_by(Program) ) %>%
...
acad_credentials <- getColleagueData( "ACAD_CREDENTIALS" ) %>%
    ...
    collect() %>%
    inner_join( ipeds_cohort %>% select(ID,Cohort_Start_Date), by = join_by(ID) ) %>%
    left_join( terms %>% select(Term_ID, Term_End_Date ), by = join_by(Term_ID) ) %>%
...
student_acad_cred <- getColleagueData( "STUDENT_ACAD_CRED" ) %>%
    ...
    distinct() %>%
    collect() %>%
    inner_join( terms %>% select(Term_ID,Term_Start_Date), by = join_by(Term_ID) ) %>%
    left_join( report_term_still_enrolled %>% select(Term_ID) %>% mutate(Enrolled_IPEDS = 1), by = join_by(Term_ID) ) %>%
    inner_join( ipeds_cohort %>% select(ID,Cohort_Start_Date), by = join_by(ID) ) %>%
...
nsc_files <- list.files( path = local_nsc_path,
    ...
    filter( `College Code/Branch` != str_glue("{fice}-{branch}") ) %>%
    ungroup() %>%
    inner_join( ipeds_cohort %>% select(ID,Cohort), by = join_by(ID) ) %>%
...
ipeds_terms <- ipeds_cohort %>%
    ...
    select(-j) %>%
    filter( Term_Start_Date >= Cohort_Start_Date ) %>%
    left_join( student_programs, by = join_by(ID, Cohort_Start_Date, IPEDS_Report, Term_ID, Term_Start_Date) ) %>%
    left_join( acad_credentials, by = join_by(ID, Term_ID, Program) ) %>%
    left_join( student_acad_cred, by = join_by(ID, Term_ID) ) %>%
    left_join( nsc_files, by = join_by(ID, Term_ID, Term_Start_Date) ) %>%
...
ipeds_terms_grads <- ipeds_terms %>%
    ...
    summarise( Grad_Level = max(Grad_Level), .by = c(ID, Term_Start_Date, Grad_Time) ) %>%
    #group_by( ID, Grad_Time, Grad_Level ) %>%
    summarise( Graduation_Date = min(Term_Start_Date), .by = c(ID, Grad_Time, Grad_Level) ) #%>%
    #ungroup()

ipeds_terms_transferred <- ipeds_terms %>%
    select( ID, Term_Start_Date, Transferred ) %>%
    filter( Transferred > 0 ) %>%
    left_join( ipeds_terms_grads, by = join_by(ID), relationship = "many-to-many" ) %>%
    ...
    summarise( Transferred_Date = min(Term_Start_Date), .by = c(ID, Transferred) ) #%>%
    #ungroup()
...
ipeds_g2_b_10 <- ipeds_terms %>%
    ...
    distinct() %>%
    inner_join( report_term_still_enrolled %>% select(Term_ID), by = join_by(Term_ID) ) %>%
    #group_by( ID ) %>%
    left_join( person %>% select(ID,IPEDS_Race_Code), by = join_by(ID) ) %>%
    #group_by( IPEDS_Status, IPEDS_Race_Code ) %>%
    mutate( student = 1 ) %>%
    summarise( Students = sprintf("%06d", sum(student) ), .by = c(IPEDS_Status, IPEDS_Race_Code) ) %>%
...
ipeds_g2_b <- ipeds_terms %>%
    ...
    distinct() %>%
    left_join( ipeds_terms_grads, by = join_by(ID), relationship = "many-to-many" ) %>%
    left_join( ipeds_terms_transferred, by = join_by(ID) ) %>%
    ...
    inner_join( report_term_still_enrolled %>% select(Term_ID), by = join_by(Term_ID) ) %>%
    #group_by( ID ) %>%
    summarise( IPEDS_Status = min(Status_Type), .by = ID ) %>%
    left_join( person %>% select(ID,IPEDS_Race_Code), by = join_by(ID) ) %>%
    #group_by( IPEDS_Status, IPEDS_Race_Code ) %>%
    mutate( student = 1 ) %>%
    summarise( Students = sprintf("%06d", sum(student) ), .by = c(IPEDS_Status, IPEDS_Race_Code) ) %>%
    ...
    mutate( UNITID = ipeds_unitid,
            ...
            Filler2 = '     ',
            Filler3 = ' ' ) %>%
    complete( LINE = c('11', '12', '30', '51'), 
              fill = list( Students = '000000', 
                           UNITID = ipeds_unitid, 
                           SURVSECT = params$report_code, 
                           PART = 'B', 
                           Filler1 = ' ', 
...
ipeds_g2_d_10 <- ipeds_terms %>%
    ...
    mutate( IPEDS_Status = '10' ) %>%
    distinct() %>%
    left_join( ta_acyr, by = join_by(ID) ) %>%
...
if (report_year < 2024) {
    ipeds_g2_e <- tibble( UNITID = ipeds_unitid,
                          SURVSECT = params$report_code,
                          PART = 'E',
                          GRGU01 = '2',
                          GRGU011 = '000000',
...
ipeds_g2_b %>%
    ungroup() %>%
    select( LINE, starts_with('GRRACE') ) %>%
    mutate_at(c('LINE'),as.integer) %>%
    mutate(across(starts_with('GRRACE'), as.integer)) %>%
    gt()
```
...
ipeds_g2_c %>%
    ungroup() %>%
    select( 'COMP10011', 'COMP10012' ) %>%
    mutate_at(c('COMP10011', 'COMP10012'),as.integer) %>%
    gt()
```
...
ipeds_g2_d %>%
    ungroup() %>%
    select( LINE, PELL=PELLGRANT_RCPT, LOAN=DIRECTLOAN_RCPT ) %>%
    mutate_at(c('LINE'),as.integer) %>%
    mutate_at(c('PELL','LOAN'), as.integer) %>%
    gt()
```

## Part E

This is only completed for reports prior to 2024.
...
if (report_year < 2024) {
    ipeds_g2_e %>%
        ungroup() %>%
        select( GRGU01, GRGU011, GRGU012 ) %>%
        mutate_at(c('GRGU01', 'GRGU011', 'GRGU012'),as.integer) %>%
        gt()
}
```
...
ipeds_g22_a %>%
    ungroup() %>%
    select( ADEXCL, COMPY4, STILLENROLLED ) %>%
    mutate_at(c('ADEXCL', 'COMPY4', 'STILLENROLLED'),as.integer) %>%
    gt()
```
...
if (WRITE_OUTPUT) {
    ...
    if (report_year < 2024) {
        write.table( data.frame(ipeds_g2_e), file.path(output_path, fn), sep="", col.names = FALSE, row.names = FALSE, quote=FALSE, append=TRUE  )
    }

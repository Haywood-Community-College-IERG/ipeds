...
aid_state_local <- c("NCCG","SCH","SSIG","STATE","NCSAG","NCLCG")  # Need to use Award_Type == 'S' for SCH
...
ipeds_saf_f %>%
    ungroup() %>%
    select( 'YEAR', 'INCOME_RANGE', 'T4AID_N', 'T4AID_G', 'T4AID_T' ) %>%
    mutate_at(c('YEAR', 'INCOME_RANGE', 'T4AID_N', 'T4AID_G', 'T4AID_T'),as.integer) %>%
    gt() %>% 
    fmt_integer( everything() )
```
...
if (WRITE_OUTPUT) {
    ...
    write.table( data.frame(ipeds_saf_d), file.path(output_path, fn), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )
    write.table( data.frame(ipeds_saf_e), file.path(output_path, fn), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )
    write.table( data.frame(ipeds_saf_f), file.path(output_path, fn), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )

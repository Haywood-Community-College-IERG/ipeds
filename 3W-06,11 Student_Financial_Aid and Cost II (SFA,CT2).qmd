---
title: "IPEDS Student Financial Aid and Cost II"
#format: pdf
format: html
#  docx:
...
# Create tables for the award codes for each of the aid groups
awards <- getColleagueData( "AWARDS" ) %>%
    select( Award_Code = AW.ID, 
            Award_Category = AW.CATEGORY, 
            Award_Type = AW.TYPE,
            Award_Description = AW.DESCRIPTION ) %>%
    collect() %>%
    mutate( Award_Category = coalesce(Award_Category, "NONE") ) %>%
    mutate( Award_Type = coalesce(Award_Type, "NONE") )

get_award_codes <- function(category_vec, aw_award_type = NULL, aw_award_categories = NULL) {
    if (is.null(aw_award_categories)) {
        aw_award_categories = c("SCH","INST")
    }
    
    if (is.null(aw_award_type)) {
        awards %>%
            filter(Award_Category %in% category_vec) %$%
            Award_Code
    } else {
        awards %>%
            filter(
                (Award_Category %in% category_vec & Award_Category %nin% aw_award_categories) |
                (Award_Category %in% aw_award_categories & Award_Type == aw_award_type) 
            ) %$%
            Award_Code
    }
}

# These are Award Categories and Codes
# Major revision - converted to use award codes instead of award categories (2025-06-27)

### TODO: This will require resubmitting 2024-2025 reported data for the SFA and CT2 reports 
###       during the 2025-2026 reporting cycle.

aid_federal_loans <- c("GSL","NDSL","PERK","PLUS","USTF")
aid_federal_loans_codes <- get_award_codes( aid_federal_loans )

aid_federal_non_pell <- c("ACG","ACG1","ACG2","IASG","SCH","SEOG") # Need to use Award_Type == 'F' for SCH
aid_federal_non_pell_codes <- get_award_codes(aid_federal_non_pell, "F")

aid_federal_pell <- c("PELL")
aid_federal_pell_codes <- get_award_codes(aid_federal_pell)

aid_federal_grant <- unique(c(aid_federal_non_pell,aid_federal_pell))
aid_federal_grant_codes <- unique(c(aid_federal_non_pell_codes, aid_federal_pell_codes))

aid_federal_ws <- c("FWS")
aid_federal_ws_codes <- get_award_codes(aid_federal_ws)

aid_federal_titleiv <- unique(c(aid_federal_non_pell,aid_federal_pell,aid_federal_ws,aid_federal_loans))
aid_federal_titleiv_codes <- unique(c(aid_federal_non_pell_codes, 
                                       aid_federal_pell_codes, 
                                       aid_federal_ws_codes, 
                                       aid_federal_loans_codes))

aid_federal_other <- c("CARES", "HRF2U","HRF3U") # CARES funds are not Title IV
aid_federal_other_codes <- get_award_codes(aid_federal_other)

aid_state_local <- c("NCCG","SCH","SSIG","STATE","NCSAG","NCLCG", "NCLLC", "NCCHD", "FTRAN", "FHOUS", "FUTIL", "FMED", "FCOA", "FCARE", "LEAF", "GEER2", "STWD", "NCNBG", "NCSTS", "CLEAF", "TALTH")  # Need to use Award_Type == 'S' for "SCH"
aid_state_local_codes <- get_award_codes(aid_state_local, "S", c("SCH","INST"))

# Added INST 2025-06-23
aid_institution <- c("INST","SCH")  # Need to use Award_Type == 'I' for "SCH" and "INST"
aid_institution_codes <- get_award_codes(aid_institution, "I")

aid_other <- c("INST","SCH","VET","OUT") # Need to use Award_Type == 'O' for "SCH" and "INST"
aid_other_codes <- get_award_codes(aid_other, "O")

# WFALT - Wells Fargo Alternative Loan
# SMALT - Sallie Mae Alternative Loan
# NCALT - North Carolina Alternative Loan
# ALT2 - All other private loans other than Wells Fargo, Sallie Mae, and North Carolina
# We don't use ALT1, KEY, or MEFA 
aid_other_loans <- c("ALT") # "WFALT","SMALT","NCALT","ALT1","ALT2","KEY","MEFA"
aid_other_loans_codes <- get_award_codes(aid_other_loans, "O", c("ALT"))

aid_loans <- unique(c(aid_federal_loans,aid_other_loans))
aid_loans_codes <- unique(c(aid_federal_loans_codes, aid_other_loans_codes))
...
aid_group_2a <- unique(c(aid_federal_ws,
                         ...
                         aid_federal_other,
                         aid_other))
aid_group_2a_codes <- unique(c(aid_federal_ws_codes,
                         aid_loans_codes,
                         aid_federal_grant_codes,
                         aid_state_local_codes,
                         aid_institution_codes,
                         aid_federal_other_codes,
                         aid_other_codes))
aid_group_2b <- unique(c(aid_loans,
                         ...
                         aid_institution,
                         aid_federal_other))
aid_group_2b_codes <- unique(c(aid_loans_codes,
                         aid_federal_grant_codes,
                         aid_state_local_codes,
                         aid_institution_codes,
                         aid_federal_other_codes))
aid_group_3 <- unique(c(aid_federal_grant,
                        aid_state_local,
                        aid_institution))
aid_group_3_codes <- unique(c(aid_federal_grant_codes,
                        aid_state_local_codes,
                        aid_institution_codes))
aid_group_4 <- unique(c(aid_federal_grant,
                        ...
                        aid_state_local,
                        aid_institution))
aid_group_4_codes <- unique(c(aid_federal_grant_codes,
                        aid_federal_ws_codes,
                        aid_federal_loans_codes,
                        aid_state_local_codes,
                        aid_institution_codes))
...
sfa_group1 <- fall_students %>%
    ...
    left_join( awards %>% select(Award_Code, Award_Type) %>% rename(Award_Type_aw = Award_Type), 
               by = "Award_Code" ) %>%
    # left_join( credential_seeking, by = c("ID", "Term_ID") ) %>% 
    ...
    mutate( Award_Type = if_else(Award_Type == "-", Award_Type_aw, Award_Type) ) %>%
    mutate( IPEDS_Award_Category_2 = case_when(
            ...
            IPEDS_Award_Category = case_when(
                Award_Code %in% aid_federal_non_pell_codes ~ "Federal grant",
                Award_Code %in% aid_federal_pell_codes ~ "Federal Pell",
                Award_Code %in% aid_federal_grant_codes ~ "Federal grant",
                Award_Code %in% aid_federal_loans_codes ~ "Federal loan",
                Award_Code %in% aid_federal_other_codes ~ "Federal other",

                Award_Code %in% aid_federal_ws_codes ~ "Federal work-study",

                Award_Code %in% aid_state_local_codes ~ "State grant",

                Award_Code %in% aid_institution_codes ~ "Institutional grant",

                Award_Code %in% aid_other_codes ~ "Other grant",  # INST / NS### or N0##

                Award_Code %in% aid_other_loans_codes ~ "Other loan",
                
                Award_Code %in% aid_veterans ~ "Veterans",
                
                Award_Code %in% c('NONE') ~ 'No FA Awards',
                TRUE ~ "UNDEFINED"
            ),
            IPEDS_Award_Veterans = if_else( Award_Code %in% aid_veterans, 
                                            "Veterans", "Not Veterans" )
        ) %>%
    mutate( IPEDS_Award_Category_Diffs = (coalesce(IPEDS_Award_Category,"BAD1") != coalesce(IPEDS_Award_Category_2,"BAD2")) ) %>% 
    select( -c( IPEDS_Award_Category_2, IPEDS_Award_Category_Diffs, Award_Type_aw ) ) %>%
    collect()
...
# Group 2a students include students awarded:
#     - Federal Work Study 
#     - Loans to students
#     - Grant or scholarship aid from the federal government, state/local government, the institution, or other sources known to the institution.
sfa_group2a_ids <- sfa_group2 %>%
    filter( Award_Code %in% aid_group_2a_codes ) %>%
    select(ID) %>%
    distinct()

# Group 2b students include students awarded:
#     - Loans to students
#     - Grant or scholarship aid from the federal government, state/local government, the institution.
sfa_group2b_ids <- sfa_group2 %>%
    filter( Award_Code %in% aid_group_2b_codes ) %>%
    select(ID) %>%
    distinct()

# Group 3 students include in-state students awarded:
#     - Grant or scholarship aid from the federal government, state/local government, the institution.
# 
# DO NOT include students who received aid only from other sources.
sfa_group3 <- sfa_group2 %>%
    filter(
        (Award_Code %in% aid_group_3_codes) #&
...
### TEST ###
# Find in-state students in Group 2 who are NOT in Group 3
excluded_students <- sfa_group2 %>%
  filter(Tuition_Type == "In-State") %>%
  anti_join(sfa_group3 %>% select(ID) %>% distinct(), by = "ID") %>%

# See their award categories and types
  select(ID, Award_Code, Award_Category, Award_Type, Award_Amount) %>%
  distinct() %>%
  arrange(ID)
### END TEST ###
...
sfa_group4 <- sfa_group2 %>%
    filter( Award_Code %in% aid_federal_titleiv_codes ) %>% 
    mutate( drop = if_else( Award_Category == "SCH" & Award_Type != "F", 1, 0) ) %>% 
    filter( drop == 0 ) %>% 
    select( ID ) %>% 
    distinct() %>% 
    left_join( sfa_group2, by = "ID" ) %>% 
    filter( Award_Code %in% aid_group_4_codes )
...
saf_a_cols <- c( UNITID = NA_character_, SURVSECT = NA_character_, PART = NA_character_, 
                 ...
                 UGAIDDGN = NA_character_, UGAIDDGPN = NA_character_, UGLNFDGN = NA_character_,
                 UGAIDDGA = NA_character_, UGAIDDGPA = NA_character_, UGLNFDGA = NA_character_
                 )
a_cols_a <- c( UGAIDA = NA_character_, UGAIDPA = NA_character_, UGLNFA = NA_character_, 
               UGAIDDGA = NA_character_, UGAIDDGPA = NA_character_, UGLNFDGA = NA_character_)
n_cols_a <- c( SCFA2 = NA_character_, SCFA2a = NA_character_, 
               UGAIDN = NA_character_, UGAIDPN = NA_character_, UGLNFN = NA_character_,
               UGAIDDGN = NA_character_, UGAIDDGPN = NA_character_, UGLNFDGN = NA_character_)
...
ipeds_saf_a <- sfa_group1 %>%
    ...
    summarise( SCFA2 = n_distinct(ID),  # IPEDS SFA Part A, Line 01
               ...
               # IPEDS SFA Part B, Line 01, Column 1
               UGAIDN = n_distinct(ID[Award_Amount > 0], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 01, Column 3
               UGAIDA = sum(Award_Amount, na.rm = TRUE),  
               # IPEDS SFA Part B, Line 01, Column 5
               UGAIDDGN = n_distinct(ID[Award_Amount > 0 & Credential_Seeker == 1], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 01, Column 7
               UGAIDDGA = sum(Award_Amount[Credential_Seeker == 1], na.rm = TRUE),  
               
               # IPEDS SFA Part B, Line 02, Column 1
               UGAIDPN = n_distinct(ID[Award_Code %in% aid_federal_pell_codes & Award_Amount > 0], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 02, Column 3
               UGAIDPA = sum(Award_Amount[Award_Code %in% aid_federal_pell_codes], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 02, Column 5
               UGAIDDGPN = n_distinct(ID[Award_Code %in% aid_federal_pell_codes & Award_Amount > 0 & Credential_Seeker == 1], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 02, Column 7
               UGAIDDGPA = sum(Award_Amount[Award_Code %in% aid_federal_pell_codes & Credential_Seeker == 1], na.rm = TRUE),  
               
               # IPEDS SFA Part B, Line 03, Column 1
               UGLNFN = n_distinct(ID[Award_Code %in% aid_federal_loans_codes & Award_Amount > 0], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 03, Column 3
               UGLNFA = sum(Award_Amount[Award_Code %in% aid_federal_loans_codes], na.rm = TRUE),
               # IPEDS SFA Part B, Line 03, Column 5
               UGLNFDGN = n_distinct(ID[Award_Code %in% aid_federal_loans_codes & Award_Amount > 0 & Credential_Seeker == 1], na.rm = TRUE),  
               # IPEDS SFA Part B, Line 03, Column 7
               UGLNFDGA = sum(Award_Amount[Award_Code %in% aid_federal_loans_codes & Credential_Seeker == 1], na.rm = TRUE),
...
#
# The Filler_ items have been moved to CT2
#
saf_b_cols <- c( UNITID = NA_character_, SURVSECT = NA_character_, PART = NA_character_, 
                 SCFA1N = NA_character_, Filler_SCFA11N = NA_character_, Filler_SCFA12N = NA_character_, Filler_SCFA13N = NA_character_,
                 Filler_SCFY1N = NA_character_,  # IPEDS SFA Part A, Line 02
                 ANYAIDN = NA_character_, ANYAIDNF = NA_character_ )
n_cols_b <- c( SCFA1N = NA_character_, ANYAIDN = NA_character_, ANYAIDNF = NA_character_ )
...
ipeds_saf_b <- sfa_group2 %>%
    ...
    summarise( SCFA1N = n_distinct(ID),  # IPEDS SFA Part A, Line 02
               ...
               ANYAIDN = n_distinct(ID[Award_Code %in% aid_group_2a_codes & Award_Amount > 0], na.rm = TRUE),  # IPEDS SFA Part A, Line 02a
               ANYAIDNF = n_distinct(ID[Award_Code %in% aid_group_2b_codes & Award_Amount > 0], na.rm = TRUE)  # IPEDS SFA Part A, Line 02b
    ) %>%
    ...
    mutate( UNITID = ipeds_unitid,
            SURVSECT = params$report_code,
            PART = 'B',
            Filler_SCFA11N = str_pad('', 6),   # These have moved to CT2 Part G
            Filler_SCFA12N = str_pad('', 6),
            Filler_SCFA13N = str_pad('', 6),
            Filler_SCFY1N = str_pad('', 6)
            ) %>%
...
#
# CT2 Part G is First-time/First-Term, Counts of students (GROUP 2)
#
ct2_g_cols <- c( UNITID = NA_character_, SURVSECT = NA_character_, PART = NA_character_, 
                 SCFA1N = NA_character_, SCFA11N = NA_character_, SCFA12N = NA_character_, SCFA13N = NA_character_ )
ct2_n_cols_g <- c( SCFA1N = NA_character_, SCFA12N = NA_character_, SCFA13N = NA_character_ )
ct2_b6_cols_b <- c( SCFA11N = NA_character_  )


ipeds_ct2_g <- sfa_group2 %>%
    
    summarise( SCFA1N = n_distinct(ID),  # IPEDS CT2 Section 2, Part A, Establish Your Student Counts, Line 01
               #SCFA11N = 0, # IPEDS CT2 Section 2, Part B, Line 02
               SCFA12N = n_distinct(ID[Tuition_Type == "In-State"], na.rm = TRUE),  # IPEDS CT2 Section 2, Part B, Line 03
               SCFA13N = n_distinct(ID[Tuition_Type == "Out-of-State"], na.rm = TRUE)  # IPEDS CT2 Section 2, Part B, Line 04
    ) %>%
    
    add_column( !!!ct2_g_cols[!names(ct2_g_cols) %in% names(.)] ) %>%
    
    mutate_at(.vars = names(ct2_n_cols_g), function(x) {return( sprintf("%06d", x) )} ) %>%
    mutate_at(.vars = names(ct2_b6_cols_b), function(x) {return( coalesce(x, "      ") )} ) %>%

    # Add additional columns as per Import Specs
    mutate( UNITID = ipeds_unitid,
            SURVSECT = params$report_code2,
            PART = 'G'
            ) %>%
    
    # Reorder the columns to be in the proper order as per Import Specs
    select( !! names(ct2_g_cols) )

if (CLEANUP) {
    # Cleanup by removing the extra data frames
    rm( ct2_g_cols, ct2_n_cols_g, ct2_b6_cols_b )
}
...
ipeds_award_categories_c02_q1_codes <- unique(c(aid_federal_grant_codes,aid_state_local_codes,aid_institution_codes,aid_federal_other_codes))
ipeds_award_categories_c02_q2_codes <- unique(c(aid_federal_grant_codes,aid_federal_non_pell_codes,aid_federal_other_codes))
ipeds_award_categories_c02_q2b_codes <- unique(c(aid_federal_non_pell_codes,aid_federal_other_codes))

ipeds_saf_c <- sfa_group2 %>%
    
    summarise( 
        # IPEDS SFA Part C, Page 2, Line 01, Column 1
        AGRNT_N = n_distinct(ID[Award_Code %in% ipeds_award_categories_c02_q1_codes & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 02, Column 1
        FGRNT_N = n_distinct(ID[Award_Code %in% ipeds_award_categories_c02_q2_codes & Award_Type == 'F' & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 02a, Column 1
        PGRNT_N = n_distinct(ID[Award_Code %in% aid_federal_pell_codes & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 02b, Column 1
        OFGRNT_N = n_distinct(ID[Award_Code %in% ipeds_award_categories_c02_q2b_codes & Award_Type == 'F' & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 03, Column 1
        SGRNT_N = n_distinct(ID[Award_Code %in% aid_state_local_codes & Award_Type == 'S' & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 04, Column 1
        IGRNT_N = n_distinct(ID[Award_Code %in% aid_institution_codes & Award_Type == 'I' & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 05, Column 1
        LOAN_N = n_distinct(ID[Award_Code %in% aid_loans_codes & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 05a, Column 1
        FLOAN_N = n_distinct(ID[Award_Code %in% aid_federal_loans_codes & Award_Amount > 0], na.rm = TRUE),
        # IPEDS SFA Part C, Page 2, Line 05b, Column 1
        OLOAN_N = n_distinct(ID[Award_Code %in% aid_other_loans_codes & Award_Amount > 0], na.rm = TRUE),  
               
        # IPEDS SFA Part C, Page 2, Line 02a, Column 3
        PGRNT_T = sum(Award_Amount[Award_Code %in% aid_federal_pell_codes], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 02b, Column 3
        OFGRNT_T = sum(Award_Amount[Award_Code %in% ipeds_award_categories_c02_q2b_codes & Award_Type == 'F'], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 03, Column 3
        SGRNT_T = sum(Award_Amount[Award_Code %in% aid_state_local_codes & Award_Type == 'S'], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 04, Column 3
        IGRNT_T = sum(Award_Amount[Award_Code %in% aid_institution_codes & Award_Type == 'I'], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 05a, Column 3
        FLOAN_T = sum(Award_Amount[Award_Code %in% aid_federal_loans_codes], na.rm = TRUE),  
        # IPEDS SFA Part C, Page 2, Line 05b, Column 3
        OLOAN_T = sum(Award_Amount[Award_Code %in% aid_other_loans_codes], na.rm = TRUE)  
    ) %>%
...
ipeds_award_categories_l04_c2_and_3 <- unique(c(aid_federal_titleiv,
                                                aid_state_local,
                                                aid_institution))
ipeds_award_categories_l04_c2_and_3_codes <- unique(c(aid_federal_titleiv_codes,
                                                      aid_state_local_codes,
                                                      aid_institution_codes))
...
ipeds_ct2_j <- sfa_group4 %>%
    ...
    summarise( 
        ...
        T4AID_G = n_distinct(ID[Award_Code %in% ipeds_award_categories_l04_c2_and_3_codes & Award_Amount > 0], na.rm = TRUE),  
        # IPEDS CT2 Part I, Line 2/3/4, Column 3
        T4AID_T = sum(Award_Amount[Award_Code %in% ipeds_award_categories_l04_c2_and_3_codes], na.rm = TRUE)  
    ) %>%
...
ipeds_saf_a %>%
    ungroup() %>%
    select( SCFA2, SCFA2a, UGAIDN, UGAIDPN, UGLNFN, UGAIDDGN, UGAIDDGPN, UGLNFDGN ) %>%
    mutate_at(c('SCFA2', 'SCFA2a', 'UGAIDN', 'UGAIDPN', 'UGLNFN', 'UGAIDDGN', 'UGAIDDGPN', 'UGLNFDGN'),as.integer) %>%
    gt() %>% 
...
ipeds_saf_a %>%
    ungroup() %>%
    select( UGAIDA, UGAIDPA, UGLNFA, UGAIDDGA, UGAIDDGPA, UGLNFDGA ) %>%
    mutate_at(c('UGAIDA', 'UGAIDPA', 'UGLNFA', 'UGAIDDGA', 'UGAIDDGPA', 'UGLNFDGA'),as.integer) %>%
    gt() %>% 
...
ipeds_saf_b %>%
    ungroup() %>%
    select( 'SCFA1N', 'ANYAIDN', 'ANYAIDNF' ) %>%
    mutate_at(c('SCFA1N', 'ANYAIDN', 'ANYAIDNF'),as.integer) %>%
    gt() %>% 
    fmt_integer( everything() )
```
...
ipeds_ct2_g %>%
    ungroup() %>%
    select( 'SCFA1N',  'SCFA11N', 'SCFA12N', 'SCFA13N' ) %>%
    mutate_at(c('SCFA1N',  'SCFA11N', 'SCFA12N', 'SCFA13N'),as.integer) %>%
    gt() %>% 
    fmt_integer( everything() )
```
...
if (WRITE_OUTPUT) {
    ...
    write.table( data.frame(ipeds_ct2_g), file.path(output_path, fn2), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE )
    write.table( data.frame(ipeds_ct2_h), file.path(output_path, fn2), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )
    write.table( data.frame(ipeds_ct2_i), file.path(output_path, fn2), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )
    write.table( data.frame(ipeds_ct2_j), file.path(output_path, fn2), sep = "", col.names = FALSE, row.names = FALSE, quote = FALSE, append = TRUE  )
}
...
if (TEST) {
    ...
    write_csv( ipeds_ct2_g, file.path(output_path, glue("ipeds_{report_year}_{fn2}_h.csv")), na = "" )
    write_csv( ipeds_ct2_h, file.path(output_path, glue("ipeds_{report_year}_{fn2}_h.csv")), na = "" )
    write_csv( ipeds_ct2_i, file.path(output_path, glue("ipeds_{report_year}_{fn2}_i.csv")), na = "" )
    write_csv( ipeds_ct2_j, file.path(output_path, glue("ipeds_{report_year}_{fn2}_j.csv")), na = "" )
}
```